...
<div class="d-flex align-items-center gap-3 pc-footer-meta">
...
if (is_user_logged_in() && (current_user_can('administrator') || current_user_can('editor') || ($current_user_id == $author_id && current_user_can('author')))) {
        ...
        // КНОПКА ОДОБРЕНИЯ (только для модераторов и только для постов на проверке)
        if ( current_user_can('edit_others_posts') && get_post_status($post_id) === 'pending' ) {
            $approve_url = wp_nonce_url( admin_url( 'admin-post.php?action=approve_post&post_id=' . $post_id ), 'approve_post_' . $post_id );
            ?>
            <a href="<?php echo esc_url($approve_url); ?>" class="fred-approve-icon" title="<?php esc_attr_e('Одобрить и опубликовать', 'esenin'); ?>">
                <div class="pc-footer-button d-flex justify-content-center align-items-center esn-approve-icon" style="color: #198754;">
                    <span class="pc-footer-button_icon">
                        <svg xmlns="http://www.w3.org/2000/svg" id="Outline" viewBox="0 0 24 24" width="18" height="18"  style="fill: var(--es-color-accent)">
                            <path stroke="none" d="M11,23c0,.552-.448,1-1,1H5c-2.757,0-5-2.243-5-5V5C0,2.243,2.243,0,5,0H13c2.757,0,5,2.243,5,5v13c0,.552-.447,1-1,1s-1-.448-1-1V5c0-1.654-1.346-3-3-3H5c-1.654,0-3,1.346-3,3v14c0,1.654,1.346,3,3,3h5c.552,0,1,.448,1,1ZM13,5H5c-.552,0-1,.448-1,1s.448,1,1,1H13c.553,0,1-.448,1-1s-.447-1-1-1Zm0,5H5c-.552,0-1,.448-1,1s.448,1,1,1H13c.553,0,1-.448,1-1s-.447-1-1-1Zm-3,5H5c-.552,0-1,.448-1,1s.448,1,1,1h5c.552,0,1-.448,1-1s-.448-1-1-1Zm13.695,1.282c-.396-.384-1.029-.374-1.414,.022l-5.107,5.276c-.545,.568-1.528,.553-2.056-.03l-2.391-2.535c-.378-.402-1.012-.42-1.414-.042s-.42,1.012-.042,1.414l2.391,2.535c1.272,1.402,3.621,1.434,4.937,.061l5.119-5.288c.384-.397,.374-1.03-.023-1.414Z"/>
                        </svg>
                    </span>
                </div>
            </a>
    ...
    <?php } 
	     ...
	     <div class="pc-footer_icon-hidden pc-footer-button d-flex justify-content-center align-items-center" title="<?php esc_attr_e('Скрытый контент', 'esenin'); ?>">
		  <span class="pc-footer-button_icon">
		    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18"><path stroke="none" d="M19,8.424V7A7,7,0,0,0,5,7V8.424A5,5,0,0,0,2,13v6a5.006,5.006,0,0,0,5,5H17a5.006,5.006,0,0,0,5-5V13A5,5,0,0,0,19,8.424ZM7,7A5,5,0,0,1,17,7V8H7ZM20,19a3,3,0,0,1-3,3H7a3,3,0,0,1-3-3V13a3,3,0,0,1,3-3H17a3,3,0,0,1,3,3Z"/><path d="M12,14a1,1,0,0,0-1,1v2a1,1,0,0,0,2,0V15A1,1,0,0,0,12,14Z"/></svg>
		  </span>
		 </div>
