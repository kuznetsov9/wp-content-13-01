...
var esnCustomizer = {
	...
	initialize: function() {
		...
		setTimeout( function() {
			esnCustomizer.util.webfonts.standard.initialize();
			esnCustomizer.util.webfonts.google.initialize();
		}, 150 );
...
esnCustomizer.initialize();

var esnCustomizer = esnCustomizer || {};

esnCustomizer = jQuery.extend( esnCustomizer, {
	/**
		...
		webfonts: {
			...
			google: {
				...
				setFonts: function() {
					...
					jQuery.post( ajaxurl, { 'action': 'esn_typography_fonts_google_all_get' }, function( response ) {
			...
			standard: {
				...
				setFonts: function() {
					...
					jQuery.post( ajaxurl, { 'action': 'esn_typography_fonts_standard_all_get' }, function( response ) {
...
wp.customize.controlConstructor[ 'typography' ] = wp.customize.Control.extend( {
	...
	renderFontSelector: function() {
		...
		data = jQuery.extend( {}, controlFontFamilies, {
			default: {
				text: esnCustomizerConfig.defaultCSSValues,
				children: [
					{ id: '', text: esnCustomizerConfig.defaultBrowserFamily },
					{ id: 'initial', text: 'initial' },
					{ id: 'inherit', text: 'inherit' }
				]
			},
			standard: {
				text: esnCustomizerConfig.standardFonts,
				children: standardFonts
			},
			google: {
				text: esnCustomizerConfig.googleFonts,
		...
		if ( value[ 'font-family' ] || '' === value[ 'font-family' ] ) {
			value[ 'font-family' ] = esnCustomizer.util.parseHtmlEntities( value[ 'font-family' ].replace( /'/g, '"' ) );
	...
	renderVariantSelector: function() {
		...
		var control = this,
			...
			data = [],
			isValid = false,
			fontType = esnCustomizer.util.webfonts.getFontType( fontFamily ),
		...
		if ( 'google' === fontType ) {
			variants = esnCustomizer.util.webfonts.google.getVariants( fontFamily );
		}
	...
	getFonts: function() {
		var control = this,
			initialGoogleFonts = esnCustomizer.util.webfonts.google.getFonts(),
			googleFonts = {},
		...
		if ( !_.isEmpty( control.params.choices.fonts.google ) ) {
			if ( 'alpha' === control.params.choices.fonts.google[ 0 ] || 'popularity' === control.params.choices.fonts.google[ 0 ] || 'trending' === control.params.choices.fonts.google[ 0 ] ) {
				...
				googleFonts = esnCustomizer.util.webfonts.google.getFonts( googleFontsSort, '', googleFontsNumber );
		...
		if ( !_.isEmpty( control.params.choices.fonts.standard ) ) {
			_.each( control.params.choices.fonts.standard, function( fontName ) {
				if ( 'undefined' !== typeof esnCustomizer.util.webfonts.standard.fonts[ fontName ] && !_.isEmpty( esnCustomizer.util.webfonts.standard.fonts[ fontName ] ) ) {
					standardFonts[ fontName ] = {};
					if ( 'undefined' !== esnCustomizer.util.webfonts.standard.fonts[ fontName ].stack && !_.isEmpty( esnCustomizer.util.webfonts.standard.fonts[ fontName ].stack ) ) {
						standardFonts[ fontName ].family = esnCustomizer.util.webfonts.standard.fonts[ fontName ].stack;
					} else {
						standardFonts[ fontName ].family = googleFonts[ fontName ];
					}
					if ( 'undefined' !== esnCustomizer.util.webfonts.standard.fonts[ fontName ].label && !_.isEmpty( esnCustomizer.util.webfonts.standard.fonts[ fontName ].label ) ) {
						standardFonts[ fontName ].label = esnCustomizer.util.webfonts.standard.fonts[ fontName ].label;
			...
			_.each( esnCustomizer.util.webfonts.standard.fonts, function( font, id ) {
				standardFonts[ id ] = {
					family: font.stack,
					label: font.label
				};
...
wp.customize.bind( 'ready', function() {
	...
	[].forEach.call( colors, function( color ) {
		...
		expand.setAttribute( 'title', esnCustomizerConfig.advanced_settings );
 ...
 */
function esn_get_alpha_value_from_color( value ) {
	var alphaVal;
 ...
 */
 function esn_update_alpha_value_on_color_control( alpha, $control, $alphaSlider, update_slider ) {
	...
	if ( update_slider ) {
		esn_update_alpha_value_on_alpha_slider( alpha, $alphaSlider );
	}
 ...
 */
function esn_update_alpha_value_on_alpha_slider( alpha, $alphaSlider ) {
...
jQuery( document ).ready( function( $ ) {
	...
	$( '.color-alpha-control' ).each( function() {
		...
		colorPickerOptions = {
			change: function( event, ui ) {
				...
				if ( defaultColor == value ) {
					alpha = esn_get_alpha_value_from_color( value );
		...
		alphaVal = esn_get_alpha_value_from_color( startingColor );
		...
		$container.find( '.min-click-zone' ).on( 'click', function() {
			esn_update_alpha_value_on_color_control( 0, $control, $alphaSlider, true );
		});
		$container.find( '.max-click-zone' ).on( 'click', function() {
			esn_update_alpha_value_on_color_control( 100, $control, $alphaSlider, true );
		});
		...
		$container.find( '.iris-palette' ).on( 'click', function() {
			...
			alpha = esn_get_alpha_value_from_color( color );
		...
		$container.find( '.button.wp-picker-clear' ).on( 'click', function() {
			...
			esn_update_alpha_value_on_alpha_slider( 100, $alphaSlider );
		});

		// Bind event handler for clicking on the 'Default' button.
		$container.find( '.button.wp-picker-default' ).on( 'click', function() {
			var alpha = esn_get_alpha_value_from_color( defaultColor );
		...
		$control.on( 'input', function() {
			var value = $( this ).val();
			var alpha = esn_get_alpha_value_from_color( value );
		...
		$alphaSlider.slider().on( 'slide', function( event, ui ) {
			...
			esn_update_alpha_value_on_color_control( alpha, $control, $alphaSlider, false );
...
wp.customize.bind( 'ready', function() {
	var esnCustomizerDependencies = {
		...
		init: function() {
			...
			_.each( window.esnCustomizerConfig.dependencies, function( requires, controlID ) {
	...
	esnCustomizerDependencies.init();
} );
