...
if ( ! class_exists( 'ESN_Customizer_Fonts_Google' ) ) {
	/**
	 * Customizer Fonts Google Class.
	 */
	final class ESN_Customizer_Fonts_Google {
		...
		public function __construct() {
			$this->google_fonts = ESN_Customizer_Fonts::get_google_fonts();
		...
		public function loop_fields() {
			...
			if ( is_array( $fields ) && $fields ) {
				foreach ( $fields as $field ) {
					...
					if ( ! isset( $value['font-family'] ) || ! ESN_Customizer_Fonts::is_google_font( $value['font-family'] ) ) {
						continue;
					}
		...
		public function enqueue_google_fonts() {
			...
			foreach ( $this->fonts_to_load as $font ) {
				...
				/**
				 * The esn_customizer_google_fonts_subset hook.
				 *
				 * @since 1.0.0
				 */
				$subset = apply_filters( 'esn_customizer_google_fonts_subset', 'latin,latin-ext,cyrillic,cyrillic-ext,vietnamese' );

				$url = "https://fonts.googleapis.com/css?family={$family}:{$weights}&subset={$subset}&display";  /* =swap */

				wp_enqueue_style( md5( $url ), $url, array(), esn_get_theme_data( 'Version' ) );
			}
		}
	}

	new ESN_Customizer_Fonts_Google();
}