...
if ( ! class_exists( 'ESN_Customizer_Fonts_Theme' ) ) {
	/**
	 * Customizer Fonts Theme Class.
	 */
	final class ESN_Customizer_Fonts_Theme {
		/**
		 * The theme fonts.
		 *
		 * @var array
		...
		public function __construct() {
			/**
			 * The esn_theme_fonts hook.
			 *
			 * @since 1.0.0
			 */
			$this->fonts_output = apply_filters( 'esn_theme_fonts', array() );
			...
			add_filter( 'esn_customizer_dynamic_css', array( $this, 'dynamic_css_fonts_stack' ) );
			add_filter( 'esn_customizer_fonts_choices', array( $this, 'customizer_fonts_choices' ), 999 );
		...
		public function set_variants_output() {
			...
			if ( is_array( $fields ) && $fields ) {
				foreach ( $fields as $field ) {
					...
					$field_value = ESN_Customizer_Helper::get_value( $field['settings'] );
			...
			if ( is_customize_preview() ) {
				...
				if ( $this->fonts_output ) {
					$fonts['fonts']['families']['theme'] = array(
						'text'     => esc_html__( 'Theme Fonts', 'esenin' ),
						'children' => array(),
					);
				}
		...
		public function enqueue_theme_fonts() {
			wp_register_style( 'es-theme-fonts', false, array(), esn_get_theme_data( 'Version' ) );

			wp_enqueue_style( 'es-theme-fonts' );

			wp_add_inline_style( 'es-theme-fonts', $this->get_styles_theme_fonts() );
		}
	}

	new ESN_Customizer_Fonts_Theme();
}