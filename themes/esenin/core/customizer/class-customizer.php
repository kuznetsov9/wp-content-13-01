...
if ( ! class_exists( 'ESN_Customizer' ) ) {
	/**
	 * Class Theme Customizer
	 */
	class ESN_Customizer {
		...
		public function __construct() {
			...
			add_filter( 'esn_customizer_field_add_setting_args', array( $this, 'field_add_setting_args' ) );
			add_filter( 'esn_customizer_field_add_control_args', array( $this, 'field_add_control_args' ) );
		}
		...
		public function customizer_register( $wp_customize ) {
			...
			$wp_customize->register_control_type( 'ESN_Customize_Typography_Control' );
			$wp_customize->register_control_type( 'ESN_Customize_Multicheck_Control' );

			// Set fields.
			foreach ( self::$fields as $field_id => $field_args ) {
				/**
				 * The esn_customizer_field_add_setting_args hook.
				 *
				 * @since 1.0.0
				 */
				$params = apply_filters( 'esn_customizer_field_add_setting_args', $field_args, $wp_customize );

				call_user_func( array( $wp_customize, 'add_setting' ), $field_id, $params );
				/**
				 * The esn_customizer_field_add_control_args hook.
				 *
				 * @since 1.0.0
				 */
				$args = apply_filters( 'esn_customizer_field_add_control_args', $field_args, $wp_customize );
				...
				switch ( $field_args['type'] ) {
					...
					case 'image':
						...
						break;
					case 'dimension':
						$wp_customize->add_control( new ESN_Customize_Dimension_Control( $wp_customize, $field_id, $args ) );
						break;
					case 'divider':
						$wp_customize->add_control( new ESN_Customize_Divider_Control( $wp_customize, $field_id, $args ) );
						break;
					case 'heading':
						$wp_customize->add_control( new ESN_Customize_Heading_Control( $wp_customize, $field_id, $args ) );
						break;
					case 'multicheck':
						$wp_customize->add_control( new ESN_Customize_Multicheck_Control( $wp_customize, $field_id, $args ) );
						break;
					case 'collapsible':
						$wp_customize->add_control( new ESN_Customize_Collapsible_Control( $wp_customize, $field_id, $args ) );
						break;
					case 'color-alpha':
						$wp_customize->add_control( new ESN_Customize_Color_Alpha_Control( $wp_customize, $field_id, $args ) );
						break;
					case 'typography':
						$wp_customize->add_control( new ESN_Customize_Typography_Control( $wp_customize, $field_id, $args ) );
						break;
					default:
		...
		public function customizer_controls_enqueue_scripts() {
			...
			wp_register_script( 'es-customizer', get_theme_file_uri( '/core/customizer/assets/customizer.js' ), array( 'jquery', 'customize-controls', 'selectWoo' ), filemtime( get_theme_file_path( '/core/customizer/assets/customizer.js' ) ), true );

			// Localize customize scripts.
			wp_localize_script( 'es-customizer', 'esnCustomizerConfig', array(
				'dependencies'         => self::$dependencies,
				'advanced_settings'    => esc_html__( 'Advanced settings', 'esenin' ),
				'noFileSelected'       => esc_html__( 'No File Selected', 'esenin' ),
				'remove'               => esc_html__( 'Remove', 'esenin' ),
				'default'              => esc_html__( 'Default', 'esenin' ),
				'selectFile'           => esc_html__( 'Select File', 'esenin' ),
				'standardFonts'        => esc_html__( 'Standard Fonts', 'esenin' ),
				'googleFonts'          => esc_html__( 'Google Fonts', 'esenin' ),
				'defaultCSSValues'     => esc_html__( 'CSS Defaults', 'esenin' ),
				'defaultBrowserFamily' => esc_html__( 'Default Browser Font-Family', 'esenin' ),
			) );

			// Enqueue customize scripts.
			wp_enqueue_script( 'es-customizer' );
			...
			wp_register_style( 'es-customizer', get_theme_file_uri( '/core/customizer/assets/customizer.css' ), array( 'selectWoo' ), filemtime( get_theme_file_path( '/core/customizer/assets/customizer.css' ) ) );

			// Enqueue customize style.
			wp_enqueue_style( 'es-customizer' );
		}
	}

	if ( ! function_exists( 'esn_customizer' ) ) {
		/**
		 * Returns instanse of the Theme Customizer class.
		 *
		 * @return object
		 */
		function esn_customizer() {
			return ESN_Customizer::get_instance();
		}
	}
	esn_customizer();
}
