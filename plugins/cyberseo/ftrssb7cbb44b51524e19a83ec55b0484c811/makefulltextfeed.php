...
// Copyright (c) 2022 Keyvan Minoukadeh
// License: AGPLv3
// Version: 3.9.12
// Date: 2022-12-31
// More info: https://www.fivefilters.org/full-text-rss/
...
// For more request parameters, see https://help.fivefilters.org/full-text-rss/usage.html

error_reporting(E_ALL ^ E_NOTICE);
libxml_use_internal_errors(true);
if (PHP_VERSION_ID < 80000) libxml_disable_entity_loader(true); // function deprecated (defaults to disabled) in PHP 8
...
// apply special SimplePie handling (double decode) for titles: https://github.com/simplepie/simplepie/issues/637
$output->setTitle(strip_tags(html_entity_decode(htmlspecialchars_decode($feed->get_title()), ENT_QUOTES | ENT_XML1, 'UTF-8')));
$output->setDescription(strip_tags(html_entity_decode($feed->get_description(), ENT_QUOTES | ENT_XML1, 'UTF-8')));
...
$output->setRelated('https://subscribe.fivefilters.org/?url='.urlencode(get_self_url()), 'Subscribe to feed');
$output->setLink($feed->get_link()); // Google Reader uses this for pulling in favicons
if ($img_url = $feed->get_image_url()) {
	$output->setImage(
		strip_tags(html_entity_decode(htmlspecialchars_decode($feed->get_title()), ENT_QUOTES | ENT_XML1, 'UTF-8')), 
		$feed->get_link(), 
		$img_url);
}
...
foreach ($items as $key => $item) {
	...
	if ($feed_item_title !== null) {
		// apply special SimplePie handling (double decode) for titles: https://github.com/simplepie/simplepie/issues/637
		$feed_item_title = strip_tags(html_entity_decode(htmlspecialchars_decode($feed_item_title), ENT_QUOTES | ENT_XML1, 'UTF-8'));
	}
						...
						if (!in_array($next_page_url, $multi_page_urls)) {
							...
							if (($response = $http->get($next_page_url, true)) && $response['status_code'] < 300) {
								...
								if (!isset($page_mime_info['action'])) {
									...
									$html = str_replace('</[>', '', $html);
									$html = convert_to_utf8($html, $response['headers']);
									if ($extractor->process($html, $next_page_url, true, true)) {
	...
	if (isset($effective_url)) {
		...
		$newitem->addElement('dc:identifier', remove_url_cruft($effective_url));
		if ($favour_effective_url) $newitem->setLink(remove_url_cruft($effective_url));
	} else {
