...
class ContentExtractor
{
	...
	public $defaultParser = 'html5php'; // must be in $allowedParsers
	...
	public function process($html, $url, $smart_tidy=true, $is_next_page=false) {
		$this->reset();
		...
		if (!$this->config->skip_json_ld()) {
			...
			if ($elems && $elems->length > 0) {
				...
				foreach ($elems as $elem) {
					...
					if (is_array($_jsonld)) {
						foreach ($_jsonld as $_jsonld_object) {
							if (is_object($_jsonld_object)) $jsonld[] = $_jsonld_object;
						}
				...
				foreach ($jsonld as $jsonld_object) {
					if (!isset($this->jsonld['date'])) $this->processDateJsonLD($jsonld_object);
					if (!isset($this->jsonld['title'])) $this->processTitleJsonLD($jsonld_object);
					if (!isset($this->jsonld['author'])) $this->processAuthorJsonLD($jsonld_object);
					if (!isset($this->jsonld['image'])) $this->processImageJsonLD($jsonld_object);
				}
			}
		}
	...
	private function cleanAuthor($author) {
		if ((strlen($author) >= 3) && (strtolower(substr($author, 0, 3)) === 'by ')) {
			$author = trim(substr($author, 3));
		}
		return $author;
	}

	private function processTitleJsonLD($jsonld) {
		if (!$this->isAcceptedJsonLdType($jsonld)) return;
		$headline = $this->getJsonLdAttribute('headline', $jsonld);
		if ($headline && is_string($headline)) $this->jsonld['title'] = $headline;
	}

	private function processDateJsonLD($jsonld) {
		if (!$this->isAcceptedJsonLdType($jsonld)) return;
		$date = $this->getJsonLdAttribute('datePublished', $jsonld);
		if (!$date) $date = $this->getJsonLdAttribute('dateCreated', $jsonld);
		if ($date && is_string($date) && strtotime($date) > 0) {
			$this->jsonld['date'] = strtotime($date);
		}
	}

	private function processAuthorJsonLD($jsonld) {
		if (!$this->isAcceptedJsonLdType($jsonld)) return;
		$author = $this->getJsonLdAttribute('author', $jsonld);
		if (!$author) return;
		if (is_string($author) && (strpos($author, '://') === false) && strlen($author) < 80) {
			// "author":"John Smith"
			...
			foreach ($authors as $author) {
				if (is_object($author)) {
					...
					if (!$author_type || !$author_name) continue;
					if ((strtolower($author_type) === 'person') && (strpos($author_name, '://') === false) && (strlen($author_name) < 40)) {
						$author_names[] = $author_name;
					}
					if (strlen(implode(', ', $author_names)) > 80) break;
				}
			}
	...
	private function processImageJsonLD($jsonld) {
		if (!$this->isAcceptedJsonLdType($jsonld)) return;
	...
	private function getJsonLdAttribute($attr, $jsonld, $type=null) {
		if (!property_exists($jsonld, $attr)) return false;
		$val = $jsonld->$attr;
	...
	private function isAcceptedJsonLdType($jsonld) {
		...
		foreach ($types as $type) {
			...
			if (in_array(strtolower($type), array('article', 'newsarticle', 'techarticle', 'reportagenewsarticle', 'blogposting'))) return true;
		}
		return false;
	}
