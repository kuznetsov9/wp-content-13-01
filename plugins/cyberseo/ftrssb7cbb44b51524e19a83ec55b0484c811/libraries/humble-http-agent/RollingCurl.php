...
class RollingCurl implements Countable {
    ...
    private function rolling_curl($window_size = null) {
        ...
        for ($i = 0; $i < $this->window_size; $i++) {
            ...
            // 2021-02-11, PHP8 compatibility, see https://github.com/marcushat/RollingCurlX/issues/45
            // $key = (string) $ch;
            $key = is_object($ch) ? (string)spl_object_id($ch) : (string) $ch;
        ...
        do {
            ...
            while ($done = curl_multi_info_read($master)) {
                ...
                if (is_callable($callback)) {
                    ...
                    $key = is_object($done['handle']) ? (string)spl_object_id($done['handle']) : (string) $done['handle'];
