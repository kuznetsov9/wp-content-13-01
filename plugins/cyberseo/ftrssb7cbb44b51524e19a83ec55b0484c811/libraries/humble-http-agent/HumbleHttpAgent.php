 ...
 * @copyright 2011-2021 Keyvan Minoukadeh
...
class HumbleHttpAgent
{
	...
	const UA_BROWSER = 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:85.0) Gecko/20100101 Firefox/85.0';
	const UA_PHP = 'PHP/7.4';
	...
	function __construct($requestOptions=null, $method=null) {
		...
		if ($this->isOpenSSLwithProblem()) {
			// TODO: Remove this temp fix for some broken sites when using OpenSSL 1.1.1f
			// e.g. problem URL: https://www.presseportal.de/rss/dienststelle_55625.rss2
			// Problem report https://github.com/FreshRSS/FreshRSS/issues/3029
			// Possible solutions https://stackoverflow.com/questions/58342699/php-curl-curl-error-35-error1414d172ssl-routinestls12-check-peer-sigalgwr
			$this->requestOptions['ssl'] = array ('cipher_list' => 'DEFAULT@SECLEVEL=1'); // default is SECLEVEL=2
		}
		...
		if ($this->isOpenSSLwithProblem()) {
			// TODO: Remove this temp fix for some broken sites when using OpenSSL 1.1.1f (see above)
			$this->httpContext['ssl'] = array('ciphers' => 'DEFAULT@SECLEVEL=1');
		}
		// HTTP cURL
		if ($this->method === self::METHOD_CURL_MULTI) {
			...
			if ($this->isOpenSSLwithProblem()) {
				// TODO: Remove this temp fix for some broken sites when using OpenSSL 1.1.1f (see above)
				$this->curlOptions[CURLOPT_SSL_CIPHER_LIST] = 'DEFAULT@SECLEVEL=1';
			}
		}
		// Use proxy?
		if (isset($this->requestOptions['proxyhost']) && $this->requestOptions['proxyhost']) {
			// This should be default, but for HTTP extension it needs to be manually set for some reason.
			// 1 = basic auth, also: CURLAUTH_BASIC, \http\Client\Curl\AUTH_BASIC
			// TODO: CURLAUTH_NTLM is a different value and possibly a good idea to support this method.
			$this->requestOptions['proxyauthtype'] = 1;
	...
	private function isOpenSSLwithProblem() {
		// OpenSSL 1.1.1f (0x1010106f) 31 Mar 2020 
		// has trouble with some (misconfigured?) SSL sites
		// e.g. problem URL: https://www.presseportal.de/rss/dienststelle_55625.rss2
		// Problem report https://github.com/FreshRSS/FreshRSS/issues/3029
		// Possible solutions https://stackoverflow.com/questions/58342699/php-curl-curl-error-35-error1414d172ssl-routinestls12-check-peer-sigalgwr
		return (defined('OPENSSL_VERSION_NUMBER') && OPENSSL_VERSION_NUMBER === 0x1010106f);
	}
